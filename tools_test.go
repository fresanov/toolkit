...
var jsonTests = []struct {
	...
	{name: "incorrect typed json", json: `{"foo": 1}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		if !e.errorExpected && err != nil {
			t.Errorf("%s: error not exptected but one received: %s", e.name, err.Error())
		}
...
func TestTools_WriteJSONS(t *testing.T) {
	var testTools Tools
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if !payload.Error {
		t.Error("error so false in JSON and it should be true")
	}

	if rr.Code != http.StatusServiceUnavailable {
		t.Errorf("wrong status returned; expected 503, but got %d", rr.Code)
	}
}
